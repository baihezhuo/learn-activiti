...
public class LoopTests {
	...
	public void deployment() throws Exception {
		...
		Deployment deploy = this.repositoryService.createDeployment()
		  .addClasspathResource("processes/loop.bpmn")
		  .deploy();
		
	}
	
	// Const 常量定义  每一个流程<都有一个对应的KEY>
	public static final String LOOP_PROCESSDEFINITIONKEY = "loop";
	...
	public void start() throws Exception {
		...
		variables.put("rate", 0.8d);	
		//启动流程
		//设置流程的发起人:  //注意 在bpmn的 start节点里 要进行设置: activiti:initiator="applyuser"
		this.identityService.setAuthenticatedUserId("001");
		//启动流程
		ProcessInstance processInstance = this.runtimeService.startProcessInstanceByKey(LOOP_PROCESSDEFINITIONKEY, businessKey, variables);
	...
	public void exec() throws Exception {
		...
		Task task = taskService.createTaskQuery().taskAssignee("005").singleResult();
